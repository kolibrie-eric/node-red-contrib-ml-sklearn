...
class DNNCTF:
    ...
    def __instantiate(self):
        self.algorithm = tf.estimator.DNNClassifier(
    ...
    def fit(self, x, y):
        ...
        if "fc" not in self.config:
            self.config["fc"] = [
                tf.feature_column.numeric_column(str(k)) for k in x.keys()
            ]
        y = [self.config["classes"].index(i) for i in y]
        self.__instantiate()
        self.algorithm.train(
            input_fn=lambda: self.__input_func(x, y), steps=self.config["steps"]
        )
        self.__save()
    ...
    def predict(self, x):
        if self.algorithm != None:
            ...
            predictions = self.algorithm.predict(input_fn=lambda: self.__input_func(x))
            length = 0
            try:
                ...
                length = len(x[0])
            return [
                self.config["classes"][p["class_ids"][0]]
                for i, p in zip(range(length), predictions)
            ]
